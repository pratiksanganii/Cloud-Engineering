...
# We will handle zip locally during the packaging step
...
if [ "$STATUS" == "Success" ]; then
    echo "Email $SENDER_EMAIL is already verified. Skipping."
else
    echo "Email $SENDER_EMAIL is not verified (Status: $STATUS). Initiating verification..."
...
if command -v zip &> /dev/null; then
    ...
    # Fallback for Windows Git Bash users missing the zip command
    echo "⚠️ 'zip' command not found. Falling back to Windows PowerShell..."
    powershell.exe -noprofile -command "Compress-Archive -Path index.js, package.json -DestinationPath ../function.zip -Force"
fi
cd ..
